...
{
    // incr[] stores the Longest Increasing Sub-sequence ending with input[i]
    int incr[n];
    for (int i = 0; i < n; i++)
    {
        incr[i] = 1;
    ...
    // decr[] stores the Longest Decreasing Sub-sequence starting with input[i]
    int decr[n];
    for (int i = 0; i < n; i++)
    {
        decr[i] = 1;
    ...
    // Required length of longest bitonic sub-array is the maximum value of
    // (incr[i] + decr[i] - 1)
